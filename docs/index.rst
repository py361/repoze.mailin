...
:mod:`repoze.mailin` defines an API, ``IMessageStore``, for plugins
which store RFC(2)822-style messages.  Plugins can be configured dynamically
via a `setuptools <http://peak.telecommunity.com/DevCenter/setuptools>`_
:term:`entry point`, which makes it possible to plug in a variety of
backing stores (e.g., `ZODB`, a relational database, or something like
the Goolge App Engine BigTable).

Within a store, messages are identified via a ``message_id``, derived
from the RFC822 ``Message-Id`` header of the message.
...
including nested RFC822 messages, within a message.  Such attachments can be
extracted via the message object.
...
.. glossary::
    ...
    entry point
        A conventional name, as defined by :mod:`setuptools`, which
        allows looking up a callable from an egg or a Python package.

    MDA
        "mail delivery agent", e.g., :command:procmail or :command:maildrop.
